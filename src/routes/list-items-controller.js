...
async function setListItem(req, res, next) {
	...
	if (!listItem) {
		res.status(404).json({ message: `No list item was found with the id of ${id}` });
		return;
	}
...
export { setListItem, getListItems, getListItem, createListItem, updateListItem, deleteListItem };
